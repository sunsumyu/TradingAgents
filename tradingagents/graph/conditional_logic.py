...
class ConditionalLogic:
    ...
    def should_continue_policy(self, state: AgentState):
        """Determine if A-share policy analysis should continue."""
        messages = state["messages"]
        last_message = messages[-1]
        if last_message.tool_calls:
            return "tools_policy"
    ...
    def should_continue_hot_money(self, state: AgentState):
        """Determine if A-share hot-money flow analysis should continue."""
        messages = state["messages"]
        last_message = messages[-1]
        if last_message.tool_calls:
    ...
    def should_continue_lockup(self, state: AgentState):
        """Determine if A-share lockup-expiry analysis should continue."""
        messages = state["messages"]
        last_message = messages[-1]
        if last_message.tool_calls:
            return "tools_lockup"
